...
const GlobalDatePickerStyle = createGlobalStyle`
  ...
  input[type="date"]::-webkit-calendar-picker-indicator:hover { /* 달력 팝업 자체 스타일링 (브라우저별 지원 제한적) */
    background-color: rgba(0, 0, 0, 0.1);
  }
  
  .date-input-container { /* 달력 팝업 크기 및 위치 조정을 위한 추가 스타일 */
    position: relative;
    display: flex;
    flex-direction: row;
    gap: 12px;
  ...
  input[type="date"]::-webkit-calendar-picker-indicator {  /* 달력 팝업이 오른쪽에서 나오도록 강제 */
    right: 0;
    left: auto;
    position: absolute;
    width: 100%;
  ...
  input[type="date"]::-webkit-datetime-edit-fields-wrapper {   /* 달력 팝업 자체 위치 조정 */
    text-align: left;
  }
  
  .date-input-container input[type="date"]:focus { /* 달력 팝업이 오른쪽에서 나타나도록 추가 스타일 */
    outline: none;
  }
  
  input[type="date"]::-webkit-calendar-picker-indicator:active {  /* 달력 팝업 위치를 오른쪽으로 강제 */
    right: 0;
    left: auto;
  }
`;
...
const TimeButton = styled.button` // 시간 선택 관련 스타일 컴포넌트들
  border: none;
  border-radius: 0;
  padding: 15px 20px;
  font-family: 'Spoqa Han Sans Neo', sans-serif;
          ...
          <ModalContent onClick={(e) => e.stopPropagation()}>
            <Header> {/* 헤더 */}
              <HeaderLeft>
                ...
                <DateSubtitle>{selectedDate ? selectedDate.toLocaleDateString('ko-KR', {
                  year: 'numeric',
                  month: 'long',
                  day: 'numeric'
            ...
            <ModalWindow> {/* 모달 창 */}
              <Form>
                ...
                {/* 날짜 선택 */}
                <div style={{display: 'flex', flexDirection: 'column', gap: '10px', marginBottom: '3%'}}>
                  <DateRowSection>
                    <DateRow>
