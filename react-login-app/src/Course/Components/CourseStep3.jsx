...
const CourseStep3 = () => {
  ...
  // 타입별 한국어 카테고리명 반환 함수
  const getCategoryNameByType = (type) => {
    const categoryNames = {
      'restaurant': '맛집',
      'tour': '관광',
      'accommodation': '숙소',
  ...
  const loadCourseData = async () => {
    ...
    try {
      ...
      if (recommendationData) {
        ...
        if (Array.isArray(data)) {
        ...
        } else if (data && data.recommendedPlaces) {
          // 단일 객체인 경우 (당일치기)
          const dayKey = 'day0';
          console.log('단일 객체 처리 - recommendedPlaces:', data.recommendedPlaces);
          
          newCourseData[dayKey] = data.recommendedPlaces.map((place, index) => {
          ...
          console.log('day0 데이터 설정 완료:', newCourseData[dayKey]);
        }
            ...
            <React.Fragment key={location.id}>
              ...
              <div className="location-info">
                ...
                <div className="location-details">
                  ...
                  <p className="location-type">{getCategoryNameByType(location.type)}</p>
