 ...
 * if (userInfo) {
 *   console.log('사용자 타입:', userInfo.type); // 'kakao', 'email' 등
 ...
 * 사용자 로그아웃을 처리하는 함수
 * 
 * OAuth2 방식으로 로그아웃을 수행하며, 저장된 모든 인증 정보를 삭제합니다.
...
export const logout = async () => {
  try {
    // 🚪 OAuth2 로그아웃 API 호출 (로컬스토리지 정리 포함)
    oauth2KakaoApi.logout();
    console.log('✅ 로그아웃 완료');
  } catch (error) {
    console.error('❌ 로그아웃 에러:', error);
    // 에러가 발생해도 로컬 토큰은 제거됨 (oauth2KakaoApi.logout에서 처리)
  }
};
 ...
 * - 카카오 로그인: 카카오 프로필의 닉네임
 * - 이메일 로그인: 이메일 주소 (추후 확장)
 * - 기본값: '사용자'
 * 
 ...
 * 
 * 로그인 방식에 따라 적절한 프로필 이미지를 반환합니다:
 * - 카카오 로그인: 카카오 프로필 이미지 또는 썸네일
 * - 기타: null (기본 이미지 사용)
 * 
